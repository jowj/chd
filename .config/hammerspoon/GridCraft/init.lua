...
M.modal = function(mods, key, actionTable, title)
    ...
    print("modal: " .. tostring(title) .. " " .. hs.inspect(mods or {}) .. " " .. tostring(key))
    ...
    if mods and key then
        modality.triggerKey = hs.hotkey.modal.new(mods, key)
        -- allow pressing the hotkey again to exit
        modality.triggerKey:bind(mods, key, function()
            modality.triggerKey:exit()
        end)
    else
        modality.triggerKey = hs.hotkey.modal.new()
    end
    -- always allow escape to exit
    modality.triggerKey:bind({}, "escape", function()
        modality.triggerKey:exit()
    end)
    ...
    modality.triggerKey.entered = function(self)
        ...
        if not ok then
            hs.alert.show("GridCraft UI failed: " .. tostring(err))
        end
    end
